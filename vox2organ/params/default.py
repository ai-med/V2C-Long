...
hyper_ps_default = {
    ...
    # To store some values in datasets at lower precision to save system memory
    'DATASET_LOW_PRECISION': False,
    ...
    # Which template to use for each data sample
    # if != STATIC, the attributes of MESH_TEMPLATE_ID are used,
    # but the path ignored
    'TEMPLATE_MODE': 'STATIC',

    # if TEMPLATER_MODE != STATIC, load templates from here
    'TEMPLATE_DIR': None,

    # optionally overwrite template mesh suffix
    'TEMPLATE_SUFFIX': None,

    # features to add to the template
    # e.g. "months_diff", "gender", template_CSV_COLUMN_NAME
    # see data.longitudinal_info.get_mappings
    'TEMPLATE_FEATURES': [],

    # if not None, load additional tempalte features from template path + suffix
    ...
    # The metrics used for correspondence evaluation
    'COR_EVAL_METRICS': [],

    # Whether to register the predicted meshes before correspondence evaluation
    'COR_EVAL_ICP': False,

    # Main validation metric according to which the best model is determined.
    'MAIN_EVAL_METRIC': 'main_eval_metric',

    # Whether to dump latent features of the graph network
    ...
    # Whether to first aggregate metrics per patient and then mean
    'AGGREGATE_PATIENT_METRICS': True,

    # Whether to evaluate on less scans for faster evaluation
    'LIMIT_EVAL_SCANS': 0,
