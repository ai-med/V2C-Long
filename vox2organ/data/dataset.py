...
def crop(image, patch_shape, center, pad_value=0, mode="constant"):
    """From https://github.com/cvlab-epfl/voxel2mesh"""
    ...
    if needs_padding and mode != "nopadding":
        if isinstance(image, np.ndarray):
            ...
            patch = np.pad(patch, pad_width, mode=mode, constant_values=pad_value)
...
def img_with_patch_size(
    ...
    if mode == "crop":
        ...
        if is_label:
            img = F.interpolate(img[None, None].float(), patch_size, mode="nearest")[
                0, 0
            ].long()
        else:
            img = F.interpolate(
...
def deform_img(img, label):
    ...
    img, label = deform_random_grid([img, label], sigma=1, points=3, order=[3, 0])
...
def sample_surface_points(y_label, n_classes, point_count=3000):
    ...
    for c in range(1, n_classes):
        ...
        raise NotImplementedError(
            "The following line follows an old coordinate convention."
        )
        surface_points_normalized = normalize_vertices_per_max_dim(
            surface_points, shape
        )
        # debug
        logger.write_scatter_plot_if_debug(
            surface_points_normalized, "../misc/surface_points.png"
        )
...
class DatasetHandler(torch.utils.data.Dataset):
    ...
    :param DataModes data_mode: TRAIN, VALIDATION, or TEST
    ...
    def get_ID_from_index(self, index):
        """Return the UID corresponding to an index in the dataset"""
        return self.ids[index]

    def get_item_from_index(self, index: int, *args, **kwargs):
        """
        An item consists in general of (template, data, labels)
    ...
    def save_ids(train_ids, val_ids, test_ids, save_dir):
        ...
        with open(filename, "w") as f:
            ...
            for idx, (patient,mapping) in enumerate(train_ids.items()):
                f.write(f"{idx}: {patient} - {mapping}\n")
            f.write("\n\n")
            f.write("##### Validation ids #####\n\n")
            for idx, (patient,mapping) in enumerate(val_ids.items()):
                f.write(f"{idx}: {patient} - {mapping}\n")
            f.write("\n\n")
            f.write("##### Test ids #####\n\n")
            for idx, (patient,mapping) in enumerate(test_ids.items()):
                f.write(f"{idx}: {patient} - {mapping}\n")
            f.write("\n\n")
